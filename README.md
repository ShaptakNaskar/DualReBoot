...
**Native reconstruction — milestone 4 in progress:** the `native-decomp` branch renders a static beach frame directly from the original assets using native C++ and GLES2, without instruction translation or the original engine binary. It now reads the **entire** scene file — 167 models, 4,347 triangles, materials, cameras, parent transforms, vertex animations, three skinned skeletons, the visibility tables and all 604 animation tracks — evaluates which models a given time of day and date show, and evaluates any track to a value. Motion, dynamic text and platform wallpaper hosts remain ahead; the preview is not yet visually equivalent to the running original app. See [build/run instructions](native-port/) and [milestone results](reports/NATIVE-PORT.md).
