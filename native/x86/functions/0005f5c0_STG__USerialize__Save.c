/* 0005f5c0 | STG::USerialize::Save */

/* STG::USerialize::Save(STG::FFileBase*, STG::UPreference_TimeOfDay const&) */

void STG::USerialize::Save(FFileBase *param_1,UPreference_TimeOfDay *param_2)

{
  undefined1 *puVar1;
  uint uVar2;
  int iVar3;
  int unaff_EBX;
  uint uVar4;
  uint local_28;
  uchar local_24 [16];
  undefined4 uStack_14;
  
  uStack_14 = 0x5f5c9;
  FUN_0002e044();
  builtin_memcpy(local_24,"PREFTODP",8);
  FFileBase::Write(param_1,local_24,8);
  local_28 = 1;
  FFileBase::Write(param_1,(uchar *)&local_28,4);
  uVar2 = *(int *)(param_2 + 0x18) - *(int *)(param_2 + 0x1c);
  local_28 = uVar2;
  FFileBase::Write(param_1,(uchar *)&local_28,4);
  if (uVar2 == 0) {
LAB_0005f6bb:
    uVar2 = *(int *)(param_2 + 0x34) - *(int *)(param_2 + 0x38);
    local_28 = uVar2;
    FFileBase::Write(param_1,(uchar *)&local_28,4);
    if (uVar2 == 0) {
LAB_0005f753:
      uVar2 = *(int *)(param_2 + 0x50) - *(int *)(param_2 + 0x54);
      local_28 = uVar2;
      FFileBase::Write(param_1,(uchar *)&local_28,4);
      if (uVar2 == 0) {
LAB_0005f7eb:
        local_28 = (**(code **)(*(int *)param_2 + 0x18))(param_2);
        FFileBase::Write(param_1,(uchar *)&local_28,4);
        for (uVar2 = 0; uVar4 = (**(code **)(*(int *)param_2 + 0x18))(param_2), uVar2 < uVar4;
            uVar2 = uVar2 + 1) {
          local_28 = *(uint *)(*(int *)(param_2 + 0x7c) + uVar2 * 4);
          FFileBase::Write(param_1,(uchar *)&local_28,4);
        }
        return;
      }
      iVar3 = *(int *)(param_2 + 0x54);
      if (iVar3 != *(int *)(param_2 + 0x50)) {
        uVar4 = 0;
        do {
          puVar1 = (undefined1 *)(iVar3 + uVar4);
          uVar4 = uVar4 + 1;
          local_24[0] = *puVar1;
          FFileBase::Write(param_1,local_24,1);
          if (uVar2 == uVar4) goto LAB_0005f7eb;
          iVar3 = *(int *)(param_2 + 0x54);
        } while (uVar4 < (uint)(*(int *)(param_2 + 0x50) - iVar3));
      }
    }
    else {
      iVar3 = *(int *)(param_2 + 0x38);
      if (*(int *)(param_2 + 0x34) != iVar3) {
        uVar4 = 0;
        do {
          puVar1 = (undefined1 *)(iVar3 + uVar4);
          uVar4 = uVar4 + 1;
          local_24[0] = *puVar1;
          FFileBase::Write(param_1,local_24,1);
          if (uVar2 == uVar4) goto LAB_0005f753;
          iVar3 = *(int *)(param_2 + 0x38);
        } while (uVar4 < (uint)(*(int *)(param_2 + 0x34) - iVar3));
      }
    }
  }
  else {
    iVar3 = *(int *)(param_2 + 0x1c);
    if (*(int *)(param_2 + 0x18) != iVar3) {
      uVar4 = 0;
      do {
        puVar1 = (undefined1 *)(iVar3 + uVar4);
        uVar4 = uVar4 + 1;
        local_24[0] = *puVar1;
        FFileBase::Write(param_1,local_24,1);
        if (uVar2 == uVar4) goto LAB_0005f6bb;
        iVar3 = *(int *)(param_2 + 0x1c);
      } while (uVar4 < (uint)(*(int *)(param_2 + 0x18) - iVar3));
    }
  }
                    /* WARNING: Subroutine does not return */
  std::__stl_throw_out_of_range((char *)((int)DWORD_ARRAY_00027b38 + unaff_EBX + 0x20b));
}

